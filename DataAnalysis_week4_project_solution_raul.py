...
def filter_by_year(statistics, year, yearid):
    ...
    # def year_function(stat):
    #    return stat[yearid] == year

    new_dic = list(filter(lambda year_selection: year_selection[yearid] == year, statistics))
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        score = formula(info, row)
        player_id = row[info["playerid"]]
        player_tup = (player_id, score)
        new_tuple.append(player_tup)

    new_tuple.sort(key=lambda pair: pair[1], reverse=True)
    answer = [new_tuple[number] for number in list(range(numplayers))]

    return answer


# baseballdatainfoAAA = {"masterfile": "Master_2016.csv",   # Name of Master CSV file
...
#
# data = read_csv_as_list_dict('batting2.csv',',','"')
# print(top_player_ids(baseballdatainfoAAA, data, batting_average, 4))
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    data_source_names = read_csv_as_list_dict(info["masterfile"], info["separator"], info["quote"])
    answer = []

    for pair in top_ids_and_stats:
        player_id = pair[0]
        score = "{:.3f}".format(pair[1])
        for row in data_source_names:
            if row[info["playerid"]] == player_id:
                first_name = row[info["firstname"]]
                last_name = row[info["lastname"]]
        word = f"{score} --- {first_name} {last_name}"
        answer.append(word)
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    data_source_statistics = read_csv_as_list_dict(
        info["battingfile"], info["separator"], info["quote"]
    )
    yearid = info["yearid"]

    data_filtered_year = filter_by_year(data_source_statistics, year, yearid)

    data_top_players_year = top_player_ids(info, data_filtered_year, formula, numplayers)

    answer = lookup_player_names(info, data_top_players_year)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in statistics:
        ...
        # print(rowid)
        # print(new_dic)
        if not row[playerid] in new_dic:
            new_sub_dic = {}
            new_sub_dic[playerid] = row[playerid]
            for field in fields:
                new_sub_dic[field] = int(row[field])
            new_dic[rowid] = new_sub_dic
        else:
            for field in fields:
                # value = 0
                # value = int(new_dic[row[playerid]][field]) + int(row[field])
                new_dic[row[playerid]][field] += int(row[field])

    return new_dic


# statistics=[{'playerID': 'braunry02', 'yearID': '2011', 'teamID': 'MIL', 'a': 1, 'b':1},
#  {'playerID': 'cabremi01', 'yearID': '2011', 'teamID': 'DET', 'a': 2, 'b':1},
#  {'playerID': 'braunry02', 'yearID': '2012', 'teamID': 'MIL', 'a': 3, 'b':1},
#  {'playerID': 'cabremi01', 'yearID': '2012', 'teamID': 'DET', 'a': 4, 'b':1}]
#
# playerid='playerID'
# fields = ['a','b']
#
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    data_filtered_player = aggregate_by_player_id(data_source_statistics, playerid, fields)
    data_filtered_player_mod = data_filtered_player.values()

    data_top_players = top_player_ids(info, data_filtered_player_mod, formula, numplayers)
    answer = lookup_player_names(info, data_top_players)
...
def test_baseball_statistics():
    ...
    top_ops_2010 = compute_top_stats_year(
        baseballdatainfo,
